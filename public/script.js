window.onload = () => {
  ...
  window.addEventListener('resize', () => {
    const sideWidth = leftSide.offsetWidth;
    mapDiv.style.left = `${sideWidth}px`;
    resizer.style.left = `${sideWidth}px`;
    debouncedUpdateMapLayout();
  });
  ...
  **********************************/
  ...
  const loadCounties = () => {
    ...
    console.log('Attempting to fetch counties from /list-counties');
    
    $.get('/list-counties')
      .done((counties) => {
        console.log('Counties received:', counties);
        
        // Validate that counties is an array
        if (!Array.isArray(counties)) {
          console.error('Expected counties to be an array, got:', typeof counties);
          counties = [];
        }
        ...
        if (counties.length === 0) {
          console.warn('No counties received from server');
        }
      ...
      .fail((xhr, status, error) => {
        console.error('Failed to load counties:', status, error);
        console.error('Server response:', xhr.responseText);
        
        //hideLoading(countyDropdown, "Error loading counties", "fas fa-exclamation-triangle");
        countyDropdown.empty();
        countyDropdown.append($('<option></option>').attr('value', "Select a County...").text("Error loading counties"));
        showToast('Failed to load counties. Please try again.', 'error');
      });
  };

  // Reset town dropdown to initial state
  const resetTownDropdown = () => {
    townDropdown.empty();
  ...
  countyDropdown.change(function () {
    ...
    if (selectedCounty === 'Select a County...') {
      ...
      townDropdown.prop('disabled', true); // Ensure it's disabled while loading
      ...
      $.get(`/list-towns/${selectedCounty}`, (towns) => {
        ...
        townDropdown.empty();
        townDropdown.append($('<option></option>').attr('value', null).text('Error loading towns'));
